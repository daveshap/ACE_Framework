...
def replace_image_prompt_with_image_url_formatted_as_markdown(message):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        futures = [executor.submit(gpt.create_image, match) for match in matches]  # Keep the order of futures the same as matches
...
def split_message_by_images(message):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        for match in re.finditer(PATTERN, message):
            ...
            # noinspection PyTypeChecker
            futures.append((executor.submit(gpt.create_image, image_prompt), len(segments) - 1))
