...
class DataType:
    ...
    def bus_direction(self):
        return RadioList(
            values=[("northbound", "Northbound"), ("southbound", "Southbound")]
        )
    ...
    def get_northbound_source(self, layer):
        ...
        return (
            "debug"
            if next_index >= len(self.settings.layers)
            else self.settings.layers[next_index]
        )
...
class RequestDataType(DataType):
    ...
    def build_layer_message(self, layer, data):
        direction = data["direction"]
        source = (
            self.get_southbound_source(layer)
            if direction == "southbound"
            else self.get_northbound_source(layer)
        )
        message = {
            "message": data["message"],
        }
...
class TelemetryDataType(DataType):
    ...
    def __init__(self, settings):
        ...
        self.namespace = RadioList(
            values=[
                (namespace, namespace)
                for namespace in self.telemetry_manager.namespace_map.keys()
            ]
        )
    ...
    def build_layer_message(self, layer, data):
        namespace = data["namespace"]
        destination = self.telemetry_manager.build_telemetry_exchange_name(
            self.telemetry_manager.namespace_root(namespace)
        )
        message = {
            "data": data["data"],
            "namespace": namespace,
        ...
        return self.message_metadata(
            message, "telmetry", "telmetry_manager", destination, "telmetry"
        )
...
class DebugAceTui:
    def __init__(self):
        ...
        self.debug_endpoint = DebugEndpoint(
            constants.DEFAULT_DEBUG_UI_ENDPOINT_PORT, self.debug_endpoint_routes
        )
        self.debug_endpoint_url = (
            f"http://localhost:{constants.DEFAULT_DEBUG_ENDPOINT_PORT}"
        )
        self.kb = KeyBindings()
        ...
        for key, data_type in self.data_types.items():
            if key in messages and messages[key]:
                for message in messages[key]:
                    self.data_dict[layer][key].append(
                        data_type.build_ui_message(message)
                    )
    ...
    def toggle_debug_state(self):
        ...
        self.add_log_entry(
            f"Setting debug state: {'enabled' if self.debug_state else 'disabled'}"
        )
    ...
    def _init_key_bindings(self):
        ...
        for layer in self.layer_numbers:
            self.kb.add(layer, filter=self.dialog_not_focused)(
                self.layer_kb_callback(layer)
            )
    ...
    def open_dialog(self, key, title):
        ...
        if existing_messages:
            ...
            buttons.append(
                Button(
                    text="Edit",
                    handler=functools.partial(self.open_message_selector, "edit"),
                )
            )
            buttons.append(
                Button(
                    text="Delete",
                    handler=functools.partial(self.open_message_selector, "delete"),
                )
            )
        self.dialog = Dialog(title=full_title, body=HSplit(fields), buttons=buttons)
    ...
    def edit(self, key, index):
        ...
        self.update_after_change(key)
        self.add_log_entry(
            f"Edited '{key}' message {index + 1} for layer: {self.active_layer_name}"
        )
    ...
    def open_message_selector(self, action):
        ...
        values = [
            (i, yaml.dump(msg, default_flow_style=False, sort_keys=True))
            for i, msg in enumerate(self.data_dict[self.active_layer_name][key])
        ]
    ...
    def delete_message(self, key, index):
        ...
        self.update_after_change(key)
        self.add_log_entry(
            f"Deleted '{key}' message {index + 1} for layer: {self.active_layer_name}"
        )
    ...
    def update_output_display(self):
        ...
        for layer in self.layer_numbers:
            if layer != self.active_layer_number:
                ...
                text.append(
                    yaml.dump(
                        self.data_dict[name], default_flow_style=False, sort_keys=True
                    )
                )
    ...
    def run(self):
        log_display_window = HSplit(
            [
                ...
                Window(
                    content=self.log_display_control, height=self.help_message_height
                ),
            ]
        )
        top_bar = VSplit(
